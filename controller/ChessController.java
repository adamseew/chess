...
public final class ChessController implements Controller {
    ...
    /**
     * ChessController class' constructor
     *
     * @param chessModel ChessModel istance pointer
     */
    ...
    public void onClick(ChessBoard caller, Point clicked) {
        ...
        if (status == Status.WON_WHITE || status == Status.WON_BLACK) {
            ...
            if (moved.getPieceType() == PieceType.PAWN && clicked.x == 0
                ...
                switch (caller.pawnPromotionDialog()) {
                    case BISHOP:
                        ...
                        break;
                    case KNIGHT:
                        chessModel.setPiece(clicked, new Knight(moved.getPieceColor()));
                        break;
                    case QUEEN:
                        chessModel.setPiece(clicked, new Queen(moved.getPieceColor()));
                        break;
                    default:
                        chessModel.setPiece(clicked, new Rook(moved.getPieceColor()));
                }
            }
            ...
            else if (isKingUnderCheck(status == Status.WAITING_MOVE_WHITE ? PieceColor.BLACK : PieceColor.WHITE, new ChessModel(chessModel))) // Signal user check condition if it happens (it was checked by calling 
            // isKingUnderCheck
            {
                caller.signalCheck();
            }
    ...
    private boolean checkmate(PieceColor color, ChessModel model) {
        for (Piece piece : model.getPieces(color)) {
            switch (piece.getPieceType()) {
                case PAWN:
                    pawn(piece.getChessBoardLocation(), model);
                    break;
                case KNIGHT:
                    knight(piece.getChessBoardLocation(), model);
                    break;
                case ROOK:
                    rook(piece.getChessBoardLocation(), model);
                    break;
                case BISHOP:
                    bishop(piece.getChessBoardLocation(), model);
                    break;
                case QUEEN:
                    queen(piece.getChessBoardLocation(), model);
                    break;
                case KING:
                    queen(piece.getChessBoardLocation(), model, 1);
            }
    ...
    private int stepDomainScanner(Point clicked, ChessModel model, Status status, int verticalStep,
        ...
        for (int i = 0; i < steps; i++) {
            ...
            if (clicked.x > 7 || clicked.x < 0 || clicked.y > 7 || clicked.y < 0) // Out of bounds, reached chess board limit, returning true specifies that 
            ...
            if (capturable.getPieceType() == PieceType.EMPTY && setDomain) {
                ...
                return -1;
            } else // Someone of the same color in the way
            {
                return 1;
            }
        }
