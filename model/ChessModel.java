...
public final class ChessModel {
		...
		for (int i = 0; i < 8; i++) {
			...
			for (Piece piece : chessModel.pieces[i]) {
				switch (piece.getPieceType()) {
					case BISHOP:
						pieces[i][j] = new Bishop(piece);
						break;
					case EMPTY:
						pieces[i][j] = new Empty(piece);
						break;
					case KING:
						pieces[i][j] = new King(piece);
						...
						break;
					case KNIGHT:
						pieces[i][j] = new Knight(piece);
						break;
					case PAWN:
						pieces[i][j] = new Pawn(piece);
						break;
					case QUEEN:
						pieces[i][j] = new Queen(piece);
						break;
					case ROOK:
						pieces[i][j] = new Rook(piece);
				}
	...
	/**
	 * Resets domain, capture domain, and setWaitingMove pointer on the current
	 * ChessModel configuration
	 */
	...
	private void updatePiecesLocations() {
		for (int i = 0; i < 8; i++) {
			...
			for (Piece piece : pieces[i]) {
				// ARFA 8/4/2015
				// Probably transcription error: calling simply Point constructor
				// as following is a non sense
				// new Point(i, j);
